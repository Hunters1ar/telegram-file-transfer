...
def get_main_reply_keyboard(is_admin: bool = False, lang: str = "en"):
    ...
    builder.adjust(2, 2, 2, 1) if is_admin else builder.adjust(2, 2, 2)
        
    return builder.as_markup(resize_keyboard=True, persistent=True)

def get_admin_reply_keyboard(lang: str = "en"):
    """Reply keyboard shown after tapping '👑 Admin Menu'. Replaces the main menu."""
    from app.clients.telegram.i18n import t
    builder = ReplyKeyboardBuilder()

    builder.button(text=t("👥 Users", lang))
    builder.button(text=t("📈 Stats", lang))
    builder.button(text=t("📢 Broadcast", lang))
    builder.button(text=t("🛠 Maintenance", lang))
    builder.button(text=t("🔙 Back", lang))
